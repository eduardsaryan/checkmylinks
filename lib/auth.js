...
// Generate email verification token
    ...
    async sendVerificationEmail(email, name, verificationToken) {
        ...
        // const verifyUrl = `https://checkmylinks.io/verify-email?token=${verificationToken}`;
        const verifyUrl = `https://checkmylinks.io/api/auth/verify-email?token=${verificationToken}`;
        ...
        try {
            await this.resend.emails.send({
                ...
                html: `
                    ...
                    <p><a href="${verifyUrl}" style="background: #2563eb; color: white; padding: 10px 20px; text-decoration: none; border-radius: 5px;">Verify Email</a></p>
    ...
    async verifyEmail(token) {
        try {
            const result = await this.pool.query(
                'SELECT id, email, name FROM users WHERE email_verification_token = $1 AND email_verification_expires > NOW()',
                [token]
            );
